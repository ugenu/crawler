...
import { Routines, Middleware } from './routine';
...
export class Crawler<T> {
  constructor(protected options: ContextOptions<T>){}

  /** the shared context object, gets refreshed each run for memory sake */
  readonly context = new Context(this.options);

  /** the runner class that we will be using to run the routines; overridable */
  runner: typeof Runner = Runner;

  private routines: Routines<T> = {
    setup: [],
    crawl: [],
    paginate: [],
  };
   ...
   * @param routine 
   */
  setup(routine: Middleware<void, T>){
   ...
   * @param routine 
   */
  crawl(routine: Middleware<void, T>){
   ...
   * @param routine 
   */
  paginate(routine: Middleware<boolean, T>){
  ...
  async run(): Promise<T>{
    ...
    let runner = new this.runner(this.routines);
    await runner.runContext(this.context);
